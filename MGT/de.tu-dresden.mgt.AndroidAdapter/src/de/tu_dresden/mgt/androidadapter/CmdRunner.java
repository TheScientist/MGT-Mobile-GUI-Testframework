...
public class CmdRunner {
	...
	public CmdRunner(TestSuite suite,
			de.tu_dresden.mgt.androidadapter.java.Class testClass) {
	...
	private String generateCsvFile(HashMap<Verdict, Integer> stats) {
		try {
			String filename = System.getProperty("user.home") + File.separator
					+ "MGT" + File.separator + "testResults.csv";
			...
			BufferedWriter outStream = new BufferedWriter(new FileWriter(
					filename, true));
			if (!exists) {
				outStream.append("Date");
				outStream.append(',');
				outStream.append("PASS");
				outStream.append(',');
				outStream.append("ERROR");
				outStream.append(',');
				outStream.append("FAIL");
				outStream.append(',');
				outStream.append("INCONCLUSIVE");
				outStream.newLine();
			}

			outStream.append(new Date().toString());
			outStream.append(',');
			outStream.append(stats.get(Verdict.PASS) + "");
			outStream.append(',');
			outStream.append(stats.get(Verdict.ERROR) + "");
			outStream.append(',');
			outStream.append(stats.get(Verdict.FAIL) + "");
			outStream.append(',');
			outStream.append(stats.get(Verdict.INCONCLUSIVE) + "");
			outStream.newLine();
	...
	public void readTestExec(BufferedReader br, String filename)
			throws IOException {
		String line;
		...
		while ((line = br.readLine()) != null) {
			...
			if (line.contains("Failure in")) {
				error = true;
				failedMethod = line.substring(line.indexOf("Failure in ") + 11,
						line.length() - 1);
				logger.warn("Found error in " + failedMethod);
			}
			if (error
					&& line.contains("at " + testClass.getTargetPackage()
							+ ".test." + suite.getName())) {
				...
				if (badStep != null && badStep instanceof Assertion) {
					...
					if (assertion.getVerdict() != null) {
						stats.put(assertion.getVerdict(),
								stats.get(assertion.getVerdict()) + 1);
						verdict = "["
								+ assertion.getVerdict().getName()
										.toUpperCase() + "]";
					} else {
				...
				console.println(verdict + " Failure in Testcase "
						+ failedMethod.substring(4) + " at Teststep ");
		...
		stats.put(Verdict.PASS, stats.get(Verdict.PASS)
				+ suite.getTestCases().size() - sum);
		console.println();
		...
		final String resulsFile = generateCsvFile(stats);
		if (resulsFile == null) {
			console.println("Results could not be written to file (see log and may file a bug)");
		} else {
			console.println("Results written to file " + resulsFile);
		}
	}
