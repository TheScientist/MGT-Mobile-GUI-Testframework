 ...
 */
public class MgtOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "de.tu_dresden.mgt.resource.mgt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "de.tu_dresden.mgt.resource.mgt.ui.occurences.declaration";
	...
	private de.tu_dresden.mgt.resource.mgt.ui.MgtTokenScanner tokenScanner;
	...
	private de.tu_dresden.mgt.resource.mgt.IMgtTextResource textResource;
	 ...
	 */
	public MgtOccurrence(de.tu_dresden.mgt.resource.mgt.IMgtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, de.tu_dresden.mgt.resource.mgt.ui.MgtTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new de.tu_dresden.mgt.resource.mgt.mopp.MgtMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(de.tu_dresden.mgt.resource.mgt.ui.MgtBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, de.tu_dresden.mgt.resource.mgt.ui.MgtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, de.tu_dresden.mgt.resource.mgt.ui.MgtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, de.tu_dresden.mgt.resource.mgt.ui.MgtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == de.tu_dresden.mgt.resource.mgt.ui.MgtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(de.tu_dresden.mgt.resource.mgt.ui.MgtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(de.tu_dresden.mgt.resource.mgt.ui.MgtOccurrence.DECLARATION_ANNOTATION_ID);
	}
