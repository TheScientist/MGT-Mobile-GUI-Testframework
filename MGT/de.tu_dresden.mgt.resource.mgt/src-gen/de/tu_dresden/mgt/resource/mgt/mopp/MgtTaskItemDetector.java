...
/**
 * The MgtTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * MgtTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class MgtTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<de.tu_dresden.mgt.resource.mgt.mopp.MgtTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<de.tu_dresden.mgt.resource.mgt.mopp.MgtTaskItem> foundItems = new java.util.ArrayList<de.tu_dresden.mgt.resource.mgt.mopp.MgtTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new de.tu_dresden.mgt.resource.mgt.mopp.MgtTaskItem(keyword, message, localLine, offset, end));
