 ...
 */
public class MgtDebuggableInterpreter<ResultType, ContextType> extends de.tu_dresden.mgt.resource.mgt.debug.AbstractMgtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private de.tu_dresden.mgt.resource.mgt.util.AbstractMgtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private de.tu_dresden.mgt.resource.mgt.IMgtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public MgtDebuggableInterpreter(de.tu_dresden.mgt.resource.mgt.util.AbstractMgtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new de.tu_dresden.mgt.resource.mgt.IMgtInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = de.tu_dresden.mgt.resource.mgt.util.MgtStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public de.tu_dresden.mgt.resource.mgt.util.AbstractMgtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		de.tu_dresden.mgt.resource.mgt.IMgtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private de.tu_dresden.mgt.resource.mgt.IMgtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		de.tu_dresden.mgt.resource.mgt.IMgtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = de.tu_dresden.mgt.resource.mgt.util.MgtEObjectUtil.getDepth(current);
		stopCondition = new de.tu_dresden.mgt.resource.mgt.IMgtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = de.tu_dresden.mgt.resource.mgt.util.MgtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = de.tu_dresden.mgt.resource.mgt.util.MgtEObjectUtil.getDepth(current) - 1;
		stopCondition = new de.tu_dresden.mgt.resource.mgt.IMgtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
