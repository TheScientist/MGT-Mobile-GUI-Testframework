...
public class MgtDebugThread extends de.tu_dresden.mgt.resource.mgt.debug.MgtDebugElement implements org.eclipse.debug.core.model.IThread, de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener {
	
	private boolean suspended = false;
	private de.tu_dresden.mgt.resource.mgt.debug.MgtDebugTarget debugTarget;
	
	public MgtDebugThread(de.tu_dresden.mgt.resource.mgt.debug.MgtDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = de.tu_dresden.mgt.resource.mgt.util.MgtStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new de.tu_dresden.mgt.resource.mgt.debug.MgtStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public de.tu_dresden.mgt.resource.mgt.debug.MgtDebugTarget getTarget() {
		return debugTarget;
	}
