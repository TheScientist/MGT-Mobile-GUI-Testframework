...
public class MgtDebugTarget extends de.tu_dresden.mgt.resource.mgt.debug.MgtDebugElement implements org.eclipse.debug.core.model.IDebugTarget, de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener {
	
	/**
	 * associated system process
	 */
	private de.tu_dresden.mgt.resource.mgt.debug.MgtDebugProcess process;
	...
	private de.tu_dresden.mgt.resource.mgt.debug.MgtDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener> eventListeners = new java.util.ArrayList<de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener>();
	
	private de.tu_dresden.mgt.resource.mgt.debug.MgtDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new de.tu_dresden.mgt.resource.mgt.mopp.MgtMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public MgtDebugTarget(de.tu_dresden.mgt.resource.mgt.debug.MgtDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new de.tu_dresden.mgt.resource.mgt.debug.MgtDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new de.tu_dresden.mgt.resource.mgt.debug.MgtDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(de.tu_dresden.mgt.resource.mgt.debug.IMgtDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new de.tu_dresden.mgt.resource.mgt.mopp.MgtMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					de.tu_dresden.mgt.resource.mgt.debug.MgtLineBreakpoint lineBreakpoint = (de.tu_dresden.mgt.resource.mgt.debug.MgtLineBreakpoint) breakpoint;
	...
	public void handleMessage(de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message) {
		try {
			if (message.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.RESUMED)) {
	...
	public de.tu_dresden.mgt.resource.mgt.debug.MgtDebugProxy getDebugProxy() {
