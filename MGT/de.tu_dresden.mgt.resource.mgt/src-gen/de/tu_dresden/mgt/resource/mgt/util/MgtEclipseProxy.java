...
public class MgtEclipseProxy {
		...
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					...
					for (Object key : keys) {
						de.tu_dresden.mgt.resource.mgt.util.MgtMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new de.tu_dresden.mgt.resource.mgt.util.MgtRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	 ...
	 */
	public de.tu_dresden.mgt.resource.mgt.mopp.MgtResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(de.tu_dresden.mgt.resource.mgt.IMgtTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new de.tu_dresden.mgt.resource.mgt.util.MgtRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new de.tu_dresden.mgt.resource.mgt.util.MgtRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, de.tu_dresden.mgt.resource.mgt.IMgtTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), de.tu_dresden.mgt.resource.mgt.MgtEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), de.tu_dresden.mgt.resource.mgt.MgtEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new de.tu_dresden.mgt.resource.mgt.util.MgtRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
