 ...
 * code).
 */
public class MgtDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private de.tu_dresden.mgt.resource.mgt.debug.AbstractMgtDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, de.tu_dresden.mgt.resource.mgt.util.MgtPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, de.tu_dresden.mgt.resource.mgt.util.MgtPair<String, Object>>();
	...
	public MgtDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = de.tu_dresden.mgt.resource.mgt.util.MgtStringUtil.encode('#', stack);
				de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message = new de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message = new de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_VARIABLES)) {
				...
				de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage message = new de.tu_dresden.mgt.resource.mgt.debug.MgtDebugMessage(de.tu_dresden.mgt.resource.mgt.debug.EMgtDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return de.tu_dresden.mgt.resource.mgt.util.MgtStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		de.tu_dresden.mgt.resource.mgt.util.MgtPair<String, Object> pair = new de.tu_dresden.mgt.resource.mgt.util.MgtPair<String, Object>(name, value);
	...
	public de.tu_dresden.mgt.resource.mgt.debug.AbstractMgtDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(de.tu_dresden.mgt.resource.mgt.debug.AbstractMgtDebuggable debuggable) {
